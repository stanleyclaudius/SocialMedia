...
const authCtrl = {
  ...
  login: async(req, res) => {
    ...
    try {
      const user = await User.findOne({email}).populate('followers followings', 'avatar username name followers followings');
      if (!user)
  ...
  generateAccessToken: async(req, res) => {
    try {
      ...
      jwt.verify(rfToken, process.env.REFRESH_TOKEN_SECRET, async(err, result) => {
        ...
        const user = await User.findById(result.id).select('-password').populate('followers followings', 'avatar username name followers followings');
        if (!user)
